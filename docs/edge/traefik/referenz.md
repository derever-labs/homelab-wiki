...
**Cache-Verhalten:** Alle Fehlerseiten werden mit `Cache-Control: no-store` ausgeliefert, Traefiks errors-Middleware reicht den Header an den Browser durch. Der Fehlerzustand hängt vom Netz des Clients ab, nicht von der URL: eine gecachte 403- oder 404-Seite überlebt sonst den VPN-Wechsel und blockiert den Zugriff bis zum Ablauf der Cache-Dauer (bis September 2026 fünf Minuten, sichtbar als «Zugriff verweigert» trotz aktivem VPN, behebbar nur durch Warten oder Löschen der Browserdaten). Ein Guard im Deploy-Playbook lehnt jede Cache-Dauer in der nginx-Config ab. Die 403-Seite bietet «Nochmals versuchen» als Primäraktion, damit nach dem Einschalten des VPN ein Tap genügt, auch im iOS-Web-Clip ohne Reload-Knopf. Definiert in `standalone-stacks/traefik-ha/configs/nginx/config/default.conf`.
