...
Diese Seite ist die **Single Source of Truth** fuer den Ist-Zustand der Homelab-Monitoring-Coverage. Sie listet pro Layer jedes Item mit Adresse, Pfad, Status, Prioritaet, Silent-Fail-Risiko und ClickUp-Task. Architektur-Hintergrund in [Monitoring](index.md), Strategie in [Monitoring: Strategie](strategie.md), Keep-Correlation-Patterns in [Monitoring: Keep-Correlations](keep-correlations.md). Stand: 2026-05-02 (Layer-3/5/8-Notes um Loki-App-Audit 2026-06-04 erweitert -- App-Container-Blindflecken).
...
| NFS-Mount-Pipeline | 5 Mounts pro Storage-Node + PBS + App-Container-Mounts | none | missing | P1->P0-Kandidat | Mount-Loss silent; Staleness-Pattern fehlt | indirekt ueber csi-health, aber kein dedizierter Mount-Loss-Alert. **Incident 2026-06-04 (~17-19:30):** NAS-NFS-State-Event machte gleichzeitig PBS-Datastore (`/mnt/vm-backup`), jellyfin (`/jellyfin/media`) und die Media-Download-Mounts (`/jellyfin/downloads`) stale. Recovery **uneinheitlich**: PBS + jellyfin erholten sich (NFSv4-State-Recovery bzw. Container-Restart), sabnzbd/sonarr **blieben stale** -> Download-Pipeline stundenlang tot, kein Alert. Beleg dass Staleness pro Consumer alarmiert werden muss (R/W-E2E-Canary je Mount). Tracking [`86ca4pd1w`](https://app.clickup.com/t/86ca4pd1w), verwandt [`86ca1gq1y`](https://app.clickup.com/t/86ca1gq1y) |
| NFS-Server-Daemon (NAS nfsd) | 10.0.0.200:2049 | uptime + checkmk | missing | P0 | nfsd-Boot-Race: nach NAS-Reboot 0 Threads / Port 2049 `Connection refused` -> ALLE NFS-Consumer (Stash, Jellyfin, Proxmox-Storage, cert/logs/docker-Mounts) hart blockiert, kein Alert | Incident 2026-05-31 (Recovery `systemctl restart nfs-server`, threads 0->128). Fehlt: (1) Connectivity-Probe TCP 2049 + `threads>0`, (2) R/W-E2E-Canary (schreibt+liest Testdatei auf Mount, timeout-gewrappt, Hysterese `for:6-8min`). Root-Cause: `nfs-server.service` ist `oneshot/RemainAfterExit` -> 0-Thread-Start wird als `active` maskiert; kein nfsd-Boot-Hook am NAS (nur `ssh-hardening-reapply` bootup-Task). Boot-Fix + Probe in [`86ca1gq1y`](https://app.clickup.com/t/86ca1gq1y). Memory `feedback_synology_nfsd_boot_race`. **2. Incident 2026-06-04 (~17-19:30):** erneuter NFS-State-Event, diesmal mit breitem App-Container-Impact (PBS-Datastore + jellyfin + Media-Download-Mounts, siehe NFS-Mount-Pipeline-Zeile) -- bestaetigt Wiederholungs-Charakter, Boot-Fix + Probe weiterhin offen |
...
| Item | Adresse | Pfad | Status | Prio | Silent-Fail | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| vm-nomad-server-04/05/06 | 10.0.2.104/.105/.106 | checkmk + influx + uptime | partial | P0 | Job-Crashloop silent, Server-Metriken fehlen | CheckMK Standard-Agent live. Kuma HTTP-Monitore `Consul/Nomad/Vault Server API 04/05/06` + Push `Nomad Token -- vm-nomad-server-04/05/06`. `inputs.nomad` Plugin nachruesten |
...
| Postgres (DRBD Single, Affinity c05) | vm-nomad-client-06 | influx | partial | P0 | Connection-Pool-Storm bekannt | `inputs.postgresql` + Synthetic-Probe. Memory `feedback_authentik_pg_connection_storm`, `project_pg_storage_bottleneck_2026`. **Loki-Audit 2026-06-04:** Jellyseerr `REFRESH MATERIALIZED VIEW` (js_* Views) scheitert taeglich mit `must be owner of materialized view` (~133/24h) -- Refresh-Job-User besitzt die Views nicht (Ownership-Drift nach DB-Restore/User-Migration); Statistik-Views veralten. Niedrig-mittel. Tracking [`86ca4pd1w`](https://app.clickup.com/t/86ca4pd1w) |
...
| Item | Adresse | Pfad | Status | Prio | Silent-Fail | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| App-Productivity-Stack (gitea, vaultwarden, paperless, kimai, n8n, obsidian-livesync, dbgate, metabase, solidtime, tandoor, vitepress-wiki) | nomad-jobs/services + databases | uptime + influx + loki | partial | P0 | container-up-but-app-broken silent (kimai-untrusted-host-loop, vaultwarden-heartbeat fehlt) | granulare Health-Probes. **Loki-Audit 2026-06-04:** couchdb (obsidian-livesync-Backend) crasht `chttpd_auth_cache` alle ~5s (`_users`-DB fehlt, ~1440/h) -- Dienst funktional (Sync laeuft), aber Log-Spam ueberdeckt echte Fehler; Fix trivial (`curl -X PUT .../_users`). Tracking [`86ca4pd1w`](https://app.clickup.com/t/86ca4pd1w) |
| App-AI/LLM (ollama, open-webui, hollama, paperless-ai/gpt) | nomad-jobs/services | influx + direct | missing | P0 | external-api-cost-spike silent (Scrapfly + Anthropic + OpenAI) | Cost-Cap-Alerting. **Loki-Audit 2026-06-04:** paperless-ai funktionslos -- `SqliteError: locking protocol` (~90/h, `isDocumentProcessed`), wahrscheinlich SQLite-auf-NFS-Locking-Inkompatibilitaet; kein Alert. Tracking [`86ca4pd1w`](https://app.clickup.com/t/86ca4pd1w) |
...
| Media-Stack (Arr-x9 + Jellyfin-Trio + Stash-Trio + YT-Downloader-x4) | nomad-jobs/media | influx + loki + direct | partial | P0 | container-up-but-app-broken silent (`special-youtube-dl + youtube-dl` Crash-Loop auf c05 ohne Alert) | NFS-Synology-91-Percent akut, Consul-critical-Forwarding fehlt. **Loki-Audit 2026-06-04:** sonarr+sabnzbd Download-Pipeline funktional tot -- stale NFS-Mount `/jellyfin/downloads` nach NFS-Event (sabnzbd `FileExistsError` Errno 17 -> `mkdir /complete` scheitert; sonarr ~60k Import-Fehler/Tag), kein Alert. jellyfin `/jellyfin/media` ESTALE (per Container-Restart erholt). prowlarr Readarr-Sync 404 (niedrig). Tracking [`86ca4pd1w`](https://app.clickup.com/t/86ca4pd1w) |
| Tools/Misc (flame, changedetection, directus-gravel, immo-monitor, immoscraper, notifiarr, telegram-relay, phdler-bot, guacamole) | nomad-jobs/services | direct + loki | partial | P0 | immoscraper-weekly silent fail | weekly-batch-heartbeat fehlt, scrapfly-cost-cap fehlt. Memory `project_immoscraper_scrapfly`. **Loki-Audit 2026-06-04:** notifiarr komplett ausgefallen seit 2026-06-03 18:42 -- `401 client uuid not linked` (~1169/24h, Restart-Loop), Discord-Notifs still tot. Consul/Kuma zeigt faelschlich `up` (HTTP 200 auf Port, API tot) -> Health-Probe truegerisch, BIND/API-Test noetig. Tracking [`86ca4pd1w`](https://app.clickup.com/t/86ca4pd1w) |
| Infrastructure-Apps (zot-registry, github-runner, nebula-sync, smtp-relay, filebrowser, redis-zot) | nomad-jobs/infrastructure | direct + loki | partial | P0 | zot-registry-down breaks alloc-pulls silent | zot-heartbeat fehlt, smtp-tx-error-rate fehlt. **Loki-Audit 2026-06-04:** zot On-Demand-Sync scheitert dauerhaft fuer `library/video-grabber` (`unauthorized`, ~192/h) -- ein Nomad-Job referenziert ein nicht (mehr) existentes Docker-Hub-Image; Job-Config fixen oder Sync-Blocklist. Niedrig (nur dieses Repo). Tracking [`86ca4pd1w`](https://app.clickup.com/t/86ca4pd1w) |
