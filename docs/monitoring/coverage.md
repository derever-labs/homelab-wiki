...
| Vault HA Cluster | vm-nomad-server-04/05/06 | influx | missing | P0 | Sealed silent bis App-Failure; Sealed -> Nomad-Template-Render-Fail -> Jobs starten nicht (Kaskade) | Sealed-Alert + Synthetic-Heartbeat. `vault-unseal.service` liest `/etc/vault.d/unseal-keys` (NICHT von Ansible deployt) -> nach Node-Rebuild fehlt das File -> Node bleibt sealed. Memory `reference_vault_unseal_token_on_disk` |
...
| LE-Cert-Renewal `ackermannprivat.ch` | traefik@vm-traefik-01 | none | missing | P0 | 60-89 Tage silent fail | Gatus cert-expiration <30d/<14d |
...
| Vault Sealed-State | vault.service | uptime | partial | P0 | nur nach Restart-Loops kritisch; Gatus `sys/health?standbyok=true` kann SEALED Standby (HTTP 429/473) nicht von gesundem Standby unterscheiden | Gatus `sys/health`. Per-Node-Sealed-Probe ergänzen |
