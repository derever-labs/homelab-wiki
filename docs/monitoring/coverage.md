...
| Infrastructure-Apps (zot-registry, github-runner, nebula-sync, smtp-relay, filebrowser, redis-zot) | nomad-jobs/infrastructure | uptime + telegraf + loki | partial | P0 | zot-registry: livez/readyz pro Node in Kuma (6 Monitore), Keep-Route `homelab-route-zot` haerter critical, Telegraf `/metrics` via `accessControl.metrics`-User -- abgedeckt 2026-05-12. Andere Infrastructure-Apps (github-runner, smtp-relay etc.) noch silent | smtp-tx-error-rate fehlt, github-runner-online-Probe fehlt |
