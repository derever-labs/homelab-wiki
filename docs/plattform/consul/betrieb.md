...
- **ACL-System mit `default_policy = deny`:** Das ACL-System ist aktiv, gebootstrappt und seit dem 19.07.2026 auf `default_policy = deny` gehärtet -- Anfragen ohne gültigen Token werden abgelehnt (API, DNS, KV). Jeder Service authentifiziert sich mit einem eigenen Token; die Umstellung erfolgte, nachdem die Soll-Liste der Service-Tokens vollständig gegen den Ist-Zustand verifiziert war. Die frühere permissive Übergangsphase (`allow`) ist damit abgelöst.
