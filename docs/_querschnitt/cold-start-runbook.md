...
6. Nomad-Server-Quorum: 3 Server, kommen nach Vault hoch (für Workload-Identity)
7. Nomad-Clients: brauchen Consul + Vault + lokales Docker
8. System-Jobs: ZOT, traefik, alloy
9. Restliche Workloads in Abhängigkeitsreihenfolge

Pro Schritt vor dem nächsten verifizieren dass die Komponente tatsächlich healthy ist. Konkrete Host-Adressen siehe [Hosts und IPs](../_referenz/hosts-und-ips.md). Für einen einfacheren Restart-Fall (alles läuft schon, nur Service-Restart-Sequenz) siehe [Cluster-Neustart](./cluster-restart.md).

## Fünf Henne-Ei-Probleme
...
Lösung: Auto-Unseal via vault-unseal.service (liest die Shamir-Keys aus `/etc/vault.d/unseal-keys`). NICHT Consul, KEIN externer Provider. Das ist die Ist-Konfiguration; bei Cluster-Recovery muss verifiziert werden dass die Keys-Datei vorhanden und gültig ist.
...
Lösung: Vault hat lokales Disk-Storage (Raft-Storage auf jedem Vault-Server-Node), nicht CSI-Volume von Nomad. Das ist die Ist-Konfiguration.

### 3. ZOT-Image-Pull bei Cold-Start

ZOT-Container laufen auf Nomad-Workers und ziehen ihr eigenes Image aus -- ZOT. Bei Cold-Start nach Cache-Verlust ist ZOT down, also kann ZOT sich selbst nicht ziehen. Plus: erstes Service-Image (alloy, traefik) braucht ZOT, das auch noch nicht läuft.

Lösung: ZOT-Bootstrap-Image preloaded auf jedem Worker per Ansible. Plus Top-20-Base-Images permanent in ZOT-S3-Bucket (statt OnDemand). Damit kommt ZOT auch ohne Hub-Pull hoch.
...
Wenn Pi-hole als Upstream-DNS Consul-Server (`*.service.consul`-Auflösung) einträgt und Consul down ist: Recursion-Loop. Jede DNS-Query timeoutet, weil Pi-hole versucht Consul anzufragen, der nicht antwortet.

Lösung: Pi-hole-Upstream auf Public-DNS (1.1.1.1, 9.9.9.9). Consul-Forwarding für `.consul`-Suffix als spezifische Zone (Pi-hole conditional forwarding feature), nicht als generischer Upstream.

::: warning
Pi-hole-Conditional-Forwarding zu Consul ist nicht aktiv. Als Workaround setzen einzelne Nomad-Job-Specs (z.B. ZOT) `dns_servers = ["1.1.1.1","8.8.8.8"]` direkt am Job.
:::

## Verifikation pro Schritt

Nach jedem Schritt vor dem nächsten:
...
- Keys-Datei `/etc/vault.d/unseal-keys` fehlt oder defekt -- dann Vault-Manual-Unseal mit Recovery-Keys
- Synology-NAS Hardware-Defekt -- ZOT/Storage kein Bootstrap möglich, NAS-Recovery aus Backup notwendig (DRBD-Tiebreaker hilft hier nicht weil S3 separat liegt)
...
- [Cluster-Neustart](./cluster-restart.md) -- einfacherer Restart-Fall ohne Cold-Start
- [Smart-Shutdown](./smart-shutdown.md) -- geplante Reboots
- [Cluster-Resilience](./cluster-resilience.md) -- SPoF-Analyse und Ausfallszenarien
