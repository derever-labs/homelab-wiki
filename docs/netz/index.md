...
- **Was, wenn der UDM Pro ausfällt?** Er ist die Basis-Infrastruktur ohne Redundanz ([UniFi Betrieb](./unifi/betrieb.md)): Routing zwischen den Segmenten, WAN-Uplink, WLAN, Port-Forwards und die `*.local`-Auflösung stehen -- alle Switches und Access Points hängen am UDM Pro und laufen ohne ihn nicht autonom weiter. Unabhängig davon läuft die DRBD-Replikation weiter -- das Thunderbolt-Netz verbindet pve01 und pve02 direkt, am Switch vorbei ([Thunderbolt-Netzwerk](./netzwerk/index.md#thunderbolt-netzwerk)). Für die Wiederherstellung gilt: erst gleiche Firmware, dann Backup-Import ([Backup und Restore](./unifi/betrieb.md#backup-und-restore)).
