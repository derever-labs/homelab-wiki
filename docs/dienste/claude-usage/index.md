...
Fällt der Poller aus, frieren die Prozentwerte auf dem Stand des letzten Laufs ein. Aktiv gemeldet wird das über den Kuma-Push-Monitor `claude-usage Poller`: Der Poller pusht nach jedem Zyklus einen Heartbeat. Kuma schlägt an, wenn der Push ganz ausbleibt, wenn kein Konto mehr Zahlen liefert, oder wenn auch nur ein Konto einen neuen Login braucht; die Push-Meldung nennt dann die betroffenen Konten. Vorübergehende Störungen wie eine Drosselung beim Token-Refresh halten den Monitor grün, solange andere Konten liefern, und stehen nur in der Meldung. Bis zum 20. September 2026 galt die Schwelle erst bei null von vier Konten, weshalb zwei tote Logins tagelang unbemerkt blieben. Die Seite selbst verschweigt es ebenfalls nicht, sondern blendet ein Staleness-Banner mit dem Alter der Daten ein. Die Reset-Countdowns bleiben in diesem Zustand korrekt, weil sie aus den mitgelieferten Reset-Zeitpunkten laufen; ein Fenster, dessen Reset bereits in der Vergangenheit liegt, leitet die Seite clientseitig als wieder frei ab.
