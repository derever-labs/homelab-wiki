...
| Daily Digest | [inbox.ackermannprivat.ch/digest](https://inbox.ackermannprivat.ch/digest) (zwei serverseitige Läufe pro Tag, dazu on-demand) |
...
Die **Web-Inbox** hat einen eigenen Host mit ausschliesslich Authentik-Routern (`public-auth@file` extern, `intern-auth@file` intern) und der Authentik-Applikation `todo-inbox` (Gruppe `admin`). Der Dienst bindet die Inbox-Routen zusätzlich an den Host (`INBOX_HOST`): Über die Bearer-Router von todo.ackermannprivat.ch sind sie nicht erreichbar. Jede Instanz bringt dabei ihren eigenen Inbox-Host mit -- weil die Authentik-Proxy-Provider pro Hostname greifen, gehört zu jedem weiteren Host eine eigene Applikation; die vorbereitete Instanz Dani nutzt inbox-dani.ackermannprivat.ch. Alle mutierenden Inbox-Aktionen tragen HMAC-Tokens (dieselbe Mechanik wie die ntfy-Buttons) plus einen exakten Origin-Check -- der Schutz hängt damit nicht am Cookie-Verhalten. Wo Aufgaben-Titel in einen Inline-Skript-Block der Digest-Seite gerendert werden (Fokus-Overlay), werden sie Unicode-escaped: Ein Titel ist fremdkontrollierter Inhalt -- er kann aus einem ClickUp-Task eines anderen Workspace-Mitglieds stammen -- und hätte sonst den Skript-Block verlassen können. Die Sicherheits-Herleitung inkl. adversarialem Challenge steht im Service-Repo (`docs/konzept.md`, Stufe 6).
