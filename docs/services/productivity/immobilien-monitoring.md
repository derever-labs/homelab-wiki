...
| :--- | :--- | :--- | :--- |
| **Homegate** | Cookie-Relay + HTTP | DataDome (Cookie nötig) | 2x täglich (07:00, 19:00) |
...
| :--- | :--- | :--- |
| Cookie Receiver | Webhook GET | Empfängt DataDome-Cookies via Bookmarklet |
...
Homegate, ImmoScout24 und Comparis nutzen DataDome als Anti-Bot-Schutz. Headless Browser werden blockiert. Workaround: DataDome-Cookie aus echtem Browser via Bookmarklet an n8n senden. Cookie ist ca. 24h gültig.
...
DataDome-Cookies pro Portal. Wird vom Bookmarklet via Webhook aktualisiert. Scraper prüfen `updated_at < 24h` vor Verwendung.

## Betrieb

### Cookie-Refresh

Das Bookmarklet muss ca. 1x pro Tag auf homegate.ch ausgeführt werden. Bei abgelaufenem Cookie stoppt der Homegate Scraper sauber (kein Crash, nur Log-Eintrag).
